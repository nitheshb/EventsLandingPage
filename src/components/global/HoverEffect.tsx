...
export const HoverEffect = ({ items, className }: { items: { title: string; description: string; imageSrc: string }[]; className?: string }) => {
  ...
  return (
    <div className={cn("grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-6 py-10", className)}>
      {items.map((item, idx) => (
        <div
          key={item.imageSrc}
          className="relative group block h-full p-2 w-full"
        ...
        >
          ...
          <Card>
            <div className="flex flex-col justify-between h-full">
              <div className="flex justify-between items-start">
                <h3 className="text-[#82e600] text-xl sm:text-2xl md:text-3xl font-semibold">
              ...
              <CardDescription className="text-white text-base sm:text-lg md:text-xl font-medium">
                {item.description}
              </CardDescription>
...
export const CardDescription = ({ className, children }: { className?: string; children: React.ReactNode }) => {
  return <p className={cn("py-6 text-zinc-400 tracking-wide leading-relaxed text-sm sm:text-base md:text-lg", className)}>{children}</p>;
};
